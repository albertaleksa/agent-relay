...
Status: v1. This document defines the protocol and behavior implemented by Agent Relay.

The Compose deployment uses PostgreSQL. The HTTP protocol, credential rules, task lifecycle, and delivery guarantees are independent of the storage backend.
...
- **PostgreSQL:** persists agents, tasks, and delivery attempts. Transactions and `FOR UPDATE SKIP LOCKED` coordinate concurrent claims across API and worker processes.
...
Claiming atomically changes the task to `processing` and creates a delivery attempt with a new claim token. Concurrent workers cannot successfully claim the same task while that lease is active. PostgreSQL uses a transaction with `FOR UPDATE SKIP LOCKED`, so workers may claim different queued tasks concurrently without overlapping claims for one task.
